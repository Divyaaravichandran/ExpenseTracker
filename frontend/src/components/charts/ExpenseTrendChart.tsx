...
const ExpenseTrendChart: React.FC<ExpenseTrendChartProps> = ({ data }) => {
  if (data.length === 0) {
    return (
      <div className="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-lg shadow-indigo-100">
  ...
  return (
    <div className="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-lg shadow-indigo-100">
      ...
      <div className="mt-4 overflow-x-auto">
        <svg width={chartWidth} height={chartHeight + 24} className="min-w-full">
          ...
          {data.map((point, index) => {
            const x = index * stepX;
            const y = chartHeight - (point.total / maxValue) * chartHeight;
            return <circle key={`${point.month}-dot`} cx={x} cy={y} r="4" fill="#4f46e5" />;
          })}
        </svg>
      </div>
      ...
      <p className="mt-2 text-xs text-slate-500">Showing the most recent 4 entries.</p>
    </div>
  );
};
